...
try:
    from dbs import DBS, traverse_list, IgnoreList
except ImportError:
    from DailyBuildSystem.dbs import DBS, traverse_list, IgnoreList
...
class QAC:
    ...
    def __init__(self, path: str):
        ...
        self._ignore_list:IgnoreList = None
    ...
    def set_ignore_list(self, ignore_list: IgnoreList):
    ...
    def generate_summary(self, output_path: str, extras: list = None, rank: int = 10) -> bool:
        ...
        if self._misra_violations is not None:
            ...
            if self._ignore_list is not None:
                for total in totals:
                    if len(totals_ranked) > rank:
                        break

                    if self._ignore_list.is_skipped_file(total[1]):
                        continue
                    else:
                        totals_ranked.append(total)
            else:
                totals_ranked = totals[:rank]

            for total in totals_ranked:
                sum = {}
...
def main() -> bool:
    def show_help():
        text = "\n\n" \
               ...
               "-i{path}    Ignore list.\n" \
               "            Load ignore list from excel file.\n" \
               "            Ignore list effects on mail only. Attached report and archiving contain\n" \
               "            any of detected files.\n" \
    ...
    qac_root_path = None
    try:
        opts, args = getopt(sys.argv[1:], "ha:b:c:r:o:qi:0:1:2:3:4:5:6:7:")
        for opt in opts:
            options[opt[0]] = opt[1]
    ...
    if options.__contains__(OPT_IGNORE):
        ignore_list = IgnoreList(options[OPT_IGNORE])
    ...
    if ignore_list is not None:
        qac.set_ignore_list(ignore_list)
    qac.analyze_misra()
