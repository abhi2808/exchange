...
class VectorCAST:
    ...
    def launch(self) -> int:
        ...
        try:
            ...
            # 2. launch VectorCAST
            p = subprocess.Popen(self._CMD_PREFIX + 'execute batch',
                                 stdout=subprocess.PIPE,
                                 universal_newlines=True,
                                 shell=True)
            while p.poll() is None:
                ...
                if output == '':
                    continue
                if error_code == 0:
                    error_code = self.__error_check(output)
    ...
    def analyze_full_report(self, full_report: str) -> Optional[dict]:
        ...
        def get_metrics(table_metrics) -> dict:
            ...
            assert RESULT_UNIT.match(headers.eq(0).text()) is not None
            assert RESULT_SUBPROGRAM.match(headers.eq(1).text()) is not None
...
def main():
    ...
    exit_code = v.generate_full_report(report_path)
    if exit_code != 0 :
        return -1
    ...
    if output_path is not None:
        v.generate_summary(output_path)
